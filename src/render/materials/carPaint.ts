...
function buildClearcoatRoughness(size = 256): CanvasTexture {
  // Subtle low-frequency variation so the clearcoat doesn't read as a uniform mirror.
  ...
  for (let i = 0; i < lowFreq.length; i++) {
    // Map 0..1 noise into 0.18..0.32 — a narrow band around the base clearcoatRoughness.
    const v = 0.18 + lowFreq[i] * 0.14;
